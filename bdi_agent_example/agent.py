...
tester = LlmAgent(
    ...
    instruction=(
        ...
        "REGLAS CRÍTICAS DE SINTAXIS AGENTSPEAK:\n"
        "- Las creencias SIEMPRE son átomos en minúscula con aridad: estado(activo). contador(0). NUNCA variables sueltas: Estado = activo. Contador = 0.\n"
        "- Para usar el valor de una creencia dentro de un plan, instánciala primero en el contexto: contador(N) & N < 10.\n"
        "- Para actualizar una creencia: primero retráctala, luego añade la nueva: -contador(N), +contador(NuevoN).\n"
        "- El separador de plan es SIEMPRE '<-', NUNCA ':-'.\n"
        "- Todas las creencias, planes y objetivos iniciales terminan obligatoriamente con punto (.).\n"
        "- Las variables empiezan con Mayúscula (N, PosX, Resultado). Los átomos y literales con minúscula (estado, pos_x, resultado).\n"
        "- Para operar aritméticamente usa 'is': Resultado is N + 1.\n"
        "- Añade siempre un plan de contingencia para cada objetivo: +!meta(_) <- .print(\"Fallo en meta\").\n"
        "- Los objetivos iniciales se declaran antes de los planes: !start.\n"
        "- Las internal actions nativas llevan punto delante: .print(), .wait(), .send(), .halt().\n"
